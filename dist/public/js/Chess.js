...
export class Chess {
        ...
        for (let i of fenArr) {
            ...
            for (let j of i) {
                ...
                if (!isNaN(t)) {
                    ...
                    temp.push(j);
                    const king = [temp.length - 1, obj.arr.length - 1];
                    if (j === 'k') {
                        obj.king['1'] = king;
                    }
                    else if (j === 'K') {
    ...
    static getAxis(x, y, gap = 0, flag = 1) {
    ...
    getPiece(x, y, gap = 0, flag = 1) {
        const total = y * 8 + x + gap * flag;
        const ori = [x, y, gap, flag];
        const end = [total % 8, Math.floor(total / 8)];
        if (total < 0 || total > 63) {
            return { v: 'o', ori, end };
        }
